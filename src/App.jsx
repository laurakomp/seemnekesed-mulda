...
import { ShoppingBag, Sprout, Calendar, MapPin, Sun, Cloud, Flame, Droplet, Package, AlertCircle, Check, Leaf, Home, Heart, Wind, Bug, ThermometerSun, Search } from 'lucide-react';
...
export default function GardenPlan() {
  ...
  const basils = [
    { id: 'b1', name: 'Itaalia basiilik', latin: 'Ocimum basilicum', note: 'klassikaline pestobasiilik', cat: 'tropical' },
    { id: 'b2', name: 'Tailandi basiilik', latin: 'Ocimum basilicum var. thyrsiflora', note: 'aniisimaitse, Aasia köök', cat: 'tropical' },
    { id: 'b3', name: 'Sidrunbasiilik', latin: 'Ocimum × citriodorum', note: 'värske sitrustemaitse', cat: 'tropical' },
    { id: 'b4', name: 'Püha basiilik', latin: 'Ocimum tenuiflorum', note: 'tulsi · meditsiiniline · kerge nelgimaitse', cat: 'tropical' },
    { id: 'b5', name: 'Kreeka basiilik', latin: 'Ocimum minimum', note: 'kompaktne, väikesed lehed', cat: 'tropical' },
    { id: 'b6', name: 'Kaneel basiilik', latin: "Ocimum basilicum 'Cinnamon'", note: 'kaneeli-vürtsikas aroom', cat: 'tropical' },
  ];
  ...
  const trays = [
    { id: 'A', mat: 'Mat 1', heat: true, title: 'Tray A — Basils', cats: ['tropical'],
      contents: ['6 basil varieties × 4 pellets = 24', 'Itaalia · Tailandi · Sidrun · Püha · Kreeka · Kaneel', '6 pellets spare'] },
    ...
    <div className="space-y-5">
      <div className="bg-white/60 rounded-2xl border border-[#d4c9b0] p-5">
        <div className="flex items-center gap-2 mb-3">
          <Sprout size={18} className="text-[#c17a5a]" />
          <p className="font-display text-lg text-[#2a3a28]">6 basil varieties</p>
        </div>
        <div className="space-y-3">
          {basils.map(b => (
            <div key={b.id} className="flex items-start gap-3">
              <div className="flex-1">
                <p className="font-body text-sm font-medium text-[#2a3a28]">{b.name}</p>
                <p className="font-body text-xs text-[#6b6558] italic">{b.latin}</p>
                <p className="font-body text-xs text-[#4a463f] mt-0.5">{b.note}</p>
              </div>
              <Tag cat={b.cat} />
            </div>
          ))}
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white/60 rounded-2xl border border-[#d4c9b0] p-5">
        ...
        <p className="font-body text-sm text-[#4a463f] leading-relaxed">
          Heat-loving plants get the warm south/west window. Cool-loving salads and brassicas get the gentler morning-sun terrace — exactly what they would prefer anyway. Bolting risk is dramatically lower without harsh afternoon sun.
        </p>
      </div>
    </div>
  );
  ...
  const SearchView = () => {
    ...
    if (!q) return (
      <div className="text-center py-16">
        <Search size={36} className="mx-auto mb-4 text-[#b5ab90]" />
        <p className="font-display text-2xl text-[#2a3a28]">Otsi taimi</p>
        <p className="font-body text-sm text-[#6b6558] mt-1">Search plants, tasks &amp; tips</p>
      </div>
    );
    ...
    basils.forEach(b => {
      if ([b.name, b.latin, b.note].some(s => s.toLowerCase().includes(q))) {
        results.push({ type: 'basil', label: b.name, sub: `${b.latin} · ${b.note}`, cat: b.cat, nav: 'seeds' });
      }
    });

    Object.values(seedPlan).forEach(section => {
      section.items.forEach(item => {
        if ([item.name, item.note].some(s => s && s.toLowerCase().includes(q))) {
          results.push({ type: 'seeds', label: item.name, sub: item.note || '', cat: item.cat, nav: 'seeds' });
        }
      });
    });

    Object.entries(shopping).forEach(([, items]) => {
      items.forEach(item => {
        if ([item.name, item.spec].some(s => s.toLowerCase().includes(q))) {
          results.push({ type: 'shopping', label: item.name, sub: item.spec, nav: 'shopping' });
        }
      });
    });
    ...
    weeks.forEach(week => {
      week.tasks.forEach(task => {
        if (task.text.toLowerCase().includes(q)) {
          results.push({ type: 'week', label: task.text, sub: `Week ${week.n} · ${week.dates}`, nav: 'schedule' });
        }
      });
    });

    wateringByPlant.forEach(p => {
      if ([p.plants, p.details].some(s => s.toLowerCase().includes(q))) {
        results.push({ type: 'water', label: p.plants, sub: p.details.length > 80 ? p.details.slice(0, 80) + '…' : p.details, cat: p.cat, nav: 'water' });
      }
    });

    troubleshooting.forEach(item => {
      if ([item.title, item.signs, item.fix].some(s => s.toLowerCase().includes(q))) {
        results.push({ type: 'trouble', label: item.title, sub: item.signs, nav: 'fix' });
      }
    });
    ...
    const typeLabels = {
      basil:    { label: 'Basil', bg: 'rgba(193,122,90,0.15)', color: '#8b4a2a' },
      seeds:    { label: 'Seeds', bg: 'rgba(122,138,110,0.15)', color: '#3d4f3b' },
      shopping: { label: 'Shop',  bg: 'rgba(168,153,104,0.18)', color: '#6e5f34' },
      week:     { label: 'Task',  bg: 'rgba(61,79,59,0.12)',    color: '#2a3a28' },
      water:    { label: 'Water', bg: 'rgba(90,122,138,0.15)',  color: '#2a4a5a' },
      trouble:  { label: 'Fix',   bg: 'rgba(139,74,42,0.12)',   color: '#8b4a2a' },
    };

    if (!unique.length) return (
      <div className="text-center py-16">
        <p className="font-display text-xl text-[#2a3a28]">No results</p>
        <p className="font-body text-sm text-[#6b6558] mt-1">Nothing found for &ldquo;{searchQuery}&rdquo;</p>
      </div>
    );

    return (
      <div className="space-y-2">
        <p className="font-body text-xs text-[#6b6558] px-1 pb-1">
          {unique.length} result{unique.length !== 1 ? 's' : ''} for &ldquo;{searchQuery}&rdquo;
        </p>
        {unique.map((r, i) => (
          <button key={i} onClick={() => setActiveTab(r.nav)}
            className="w-full bg-white/60 rounded-2xl border border-[#d4c9b0] p-4 text-left hover:bg-white/80 transition">
            <div className="flex items-start justify-between gap-2 mb-1">
              <p className="font-display text-base text-[#2a3a28] leading-tight flex-1">{r.label}</p>
              <span className="font-body text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0"
                style={{ backgroundColor: typeLabels[r.type]?.bg, color: typeLabels[r.type]?.color }}>
                {typeLabels[r.type]?.label}
              </span>
            </div>
            {r.sub && <p className="font-body text-xs text-[#6b6558] leading-relaxed">{r.sub}</p>}
            {r.cat && <div className="mt-2"><Tag cat={r.cat} /></div>}
          </button>
        ))}
      </div>
    );
    ...
    <div className="min-h-screen paper-bg font-body pb-24">
      ...
      {activeTab === 'search' && (
        <div className="px-5 pb-3">
          <div className="relative">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#b5ab90]" />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="püha basiilik, kale, aphids…"
              autoFocus
              className="w-full bg-white/80 border border-[#d4c9b0] rounded-xl pl-9 pr-4 py-3 font-body text-sm text-[#2a3a28] placeholder-[#b5ab90] focus:outline-none focus:border-[#7a8a6e]"
            />
          </div>
        </div>
      )}
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex flex-col items-center gap-0.5 px-1 py-2 rounded-xl transition flex-1 ${
                  active ? 'bg-[#3d4f3b] text-[#f5f1e8]' : 'text-[#6b6558]'
                }`}>
                <Icon size={15} />
                <span className="font-body text-[8px] font-medium">{tab.label}</span>
              </button>
            );
          })}
        </div>
